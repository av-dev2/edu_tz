            ...
            for student_row in student_details:
                if (
                    ...
                    if st_name not in first_installment_list:
                        st_name.update({"paid_amount1": student_row["grand_total"] - student_row["outstanding_amount"]})
                        ...
                        st_name.update({"paid_amount3": student_row["grand_total"] - student_row["outstanding_amount"]})
                        third_installment_list.append(st_name)

                    elif st_name not in fourth_installment_list:
                        st_name.update({"paid_amount4": student_row["grand_total"] - student_row["outstanding_amount"]})
                        fourth_installment_list.append(st_name)

                    else:
                        st_name.update({"paid_amount5": student_row["grand_total"] - student_row["outstanding_amount"]})
                        fifth_installment_list.append(st_name)

            st_name.update(
                {"total_amount_to_be_paid": total_amount_to_be_paid, "outstanding_amount": total_unpaid_amount}
            )
